...
export default function CurrencySelectionPage() {
  ...
  useEffect(() => {
    ...
    const fetchCurrencies = async () => {
      ...
      try {
        setIsLoading(true)
        const response = await fetch(`https://dev.wellnexai.com/api/currency/list`, {
          method: 'GET',
          headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://dev.wellnexai.com/api/subscription/update-preferred-currency`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
