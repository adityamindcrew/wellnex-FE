...
export default function CurrencySelectionPage() {
  ...
  useEffect(() => {
    ...
    const fetchCurrencies = async () => {
      ...
      try {
        setIsLoading(true)
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/currency/list`, {
          method: 'GET',
          headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/subscription/update-preferred-currency`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
