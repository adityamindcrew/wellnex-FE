...
export default function CurrencySelectionPage() {
  ...
  useEffect(() => {
    ...
    const fetchCurrencies = async () => {
      ...
      try {
        setIsLoading(true)
        const response = await fetch(`https://13.61.105.209/api/currency/list`, {
          method: 'GET',
          headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://13.61.105.209/api/subscription/update-preferred-currency`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
