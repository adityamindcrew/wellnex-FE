...
export default function PaymentMethodSelection() {
  ...
  const fetchPaymentMethods = async () => {
    try {
      setLoading(true)
      const response = await fetch("https://13.61.105.209/api/subscription/cards", {
        headers: {
      ...
      // Transform API data to match our PaymentMethod type
      const transformedMethods = data.map((card: any) => ({
        id: card.id,
        type: card.brand.toLowerCase() as "visa" | "mastercard" | "applepay",
        lastFour: card.last4,
  ...
  const handleProceed = () => {
    if (selectedMethod) {
      router.push(`/payment/details?cardId=${selectedMethod}`)
    }
  }
      ...
      <div className="flex flex-col items-center space-y-6">
        ...
        <div className="w-full space-y-4">
          {paymentMethods.length === 0 ? (
            <div className="text-center text-gray-500">No saved payment methods found</div>
          ) : (
        ...
        <button
          ...
          className={`w-full rounded-md py-3 text-center text-white transition-colors ${
            selectedMethod ? "bg-black hover:bg-black/90" : "bg-gray-400 cursor-not-allowed"
          }`}
        >
          {paymentMethods.length === 0 ? "Add New Payment Method" : "Proceed to Enter Payment Details"}
        </button>
      </div>
    </div>
  )
