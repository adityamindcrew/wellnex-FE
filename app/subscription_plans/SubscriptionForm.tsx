...
const SubscriptionForm = ({ priceId }: { priceId: string }) => {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                const subscriptionResponse = await fetch(`https://13.61.105.209/api/subscription/status`, {
                    headers: {
                ...
                // Fetch saved cards
                const cardsResponse = await fetch(`https://13.61.105.209/api/subscription/cards`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
                    }
                });
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await fetch(`https://13.61.105.209/api/subscription/create`, {
                method: 'POST',
                headers: {
    ...
    const confirmRemoveCard = async () => {
        try {
            const response = await fetch(`https://13.61.105.209/api/subscription/cards/${cardToRemove}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
