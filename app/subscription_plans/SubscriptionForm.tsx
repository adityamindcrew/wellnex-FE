...
const SubscriptionForm = ({ priceId }: { priceId: string }) => {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                const subscriptionResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/subscription/status`, {
                    headers: {
                ...
                // Fetch saved cards
                const cardsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/subscription/cards`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
                    }
                });
    ...
    const confirmRemoveCard = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/subscription/cards/${cardToRemove}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
