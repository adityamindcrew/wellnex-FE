...
export default function Header() {
  ...
  useEffect(() => {
    const fetchBusinessDetail = async () => {
      ...
      const response = await fetch(`https://dev.wellnexai.com/api/business/getBusinessDetail`, {
        method: 'POST',
        headers: {
    ...
    <header className="flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4 md:px-6">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <div className="h-8 w-8 md:h-10 md:w-10 overflow-hidden rounded-full bg-gray-200">
          <img
            src={business?.logo ? `https://dev.wellnexai.com/uploads/business-logos/${business.logo}` : "/avatar.png"}
